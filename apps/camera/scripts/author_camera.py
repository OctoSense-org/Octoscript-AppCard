#!/usr/bin/env python3
"""Author the Camera flow from the storyboard spec: manifest, atlas intake, frozen
references, native contracts, reviewed semantic maps, control actions and the
compiled L0 scenes. The atlas is a deterministic Chromium render of the same
spec (scripts/design.py, measured from the Mate 70 Air), so measured crops and
authored geometry agree; intake still validates and freezes the PNG bytes."""
...
PROVIDER = 'Authored HTML storyboard measured from the Mate 70 Air camera app, rendered by headless Chromium through Playwright; no generative image model'
...
CARDS = [('camera-flash-02', '2', 'flash_panel', 'camera'), ('camera-toolbox-10', '10', 'box', 'camera')]

def manifest(scenes):
    crops, size = atlas_layout(scenes)
    return dict(schema_version=1, id='camera', artboard=[406, 776], locales=['en', 'cn'],
...
def author(scene, record):
    ...
    root = dict(t='stack', id='page', x=0, y=0, w=406, h=776, variant='surface', bg=col(design.BLACK), c=[])
    ...
    for n in scene.nodes:
        ...
        if kind == 'stack':
            ...
            path.write_text(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="{viewbox}" fill="none" stroke="#{n["color"][:6]}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="color:#{n["color"][:6]}">{SVG[n["icon"]]}</svg>')
    ...
    contract = dict(schema_version=1, id=scene.id, app='camera', number=scene.n, title=scene.title['cn'], artboard=[406, 776], font_family='Noto Sans SC',
                    structure='Mate 70 Air camera app screen, measured on the device: viewfinder placeholder, top chrome, zoom bar, mode bar, foot bar, panels',
                    content_source='Device captures of the HUAWEI Mate 70 Air camera (HarmonyOS 6.1); the viewfinder is a live camera texture at runtime',
                    palette=dict(name='Mate camera', page='#000000', panel='#202020', ink='#ffffff', accent='#e2362c'), graphics={}, tree=root)
    ...
    save(d / 'mapped.json', dict(schema_version=1, reference_sha256=refhash, tree=copy.deepcopy(root),
         ...
         limits=['The reference is a deterministic render of the same spec, measured from device layout dumps, not an independent design source', 'The viewfinder is a placeholder surface; no native capture or visual approval yet']))
    ...
    for entry in semantic['elements']:
        if entry['id'] in assets:
            ...
            entry.update(role='card', decision='reviewed', basis='Explicit independently owned panel surface, native Label/Button children')
    ...
    save(d / 'source-measurements.json', dict(atlas_sha256=sha(ROOT / 'source/atlas.png'), reference_sha256=refhash, source_crop=record['crop'], transform=tr,
         reviewed_source=True, text=measure, limits=['The storyboard is authored from device measurements, so measurement is a consistency check of the render against the spec, not OCR of an unknown image', 'No native visual acceptance']))
    report = preflight(d)
...
if __name__ == '__main__':
    ...
    if not (ROOT / 'cards/camera-01').exists(): prepare(path, ROOT, ROOT / 'pipeline-output/intake')
    bundle = {str(s.n): author(s, rec) for s, rec in zip(scenes, doc['scenes'])}
    save(ROOT / 'source/copy.json', bundle)
